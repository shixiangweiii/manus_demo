...
each phase of execution: DAG planning, super-step parallel execution,
per-node validation, reflection, etc.
启动多智能体流水线，提供 Rich 控制台 UI，实时展示执行的每个阶段：
DAG 规划、Super-step 并行执行、逐节点验证、反思等。

v2: Updated UI for DAG-based execution with Rich Tree visualization,
    super-step progress, and node state transitions.
v2: 针对 DAG 执行更新 UI，添加 Rich Tree 可视化、
    Super-step 进度展示和节点状态转移提示。
"""
...
from schema import NodeType, Reflection, StepResult, TaskEdge, TaskNode
...
async def run_interactive() -> None:
    ...
    """
    console.print(Panel(
        "[bold]Manus Demo v2[/bold] - DAG-based Multi-Agent System\n\n"
        "This demo implements:\n"
        "  [cyan]1.[/cyan] Hierarchical planning (Goal -> SubGoals -> Actions)\n"
        "  [cyan]2.[/cyan] DAG execution with parallel super-steps\n"
        "  [cyan]3.[/cyan] Per-node exit criteria validation\n"
        "  [cyan]4.[/cyan] Node state machine (PENDING->READY->RUNNING->COMPLETED/FAILED)\n"
        "  [cyan]5.[/cyan] Conditional branches and rollback\n"
        "  [cyan]6.[/cyan] Partial replanning (failed subtree only)\n"
        "  [cyan]7.[/cyan] Centralized DAGState (inspired by LangGraph)\n"
        "  [cyan]8.[/cyan] Short-term & long-term memory + knowledge retrieval\n\n"
