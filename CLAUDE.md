...
- **Current version**: v13.0 + Wave-1..7 SubAgent overhaul (helper-shared ReAct loops, runtime-built system prompts, per-caller token attribution)
...
- **`react/`** — ReActEngine (canonical ReAct loop; v12 removed legacy `_react_loop`; runs independent tool_calls concurrently via `asyncio.gather`); **`react/tool_call_helpers.py`** (Wave-1) exports `attribute_caller` / `classify_result` / `truncate_for_llm` + `RATE_LIMITED_MARKER` — shared by ReActEngine, GoalDrivenPlanner, EmergentPlanner legacy so set_caller / 3-state classification / TOOL_RESULT_TRUNCATION_LIMIT behavior never drifts again
- **`llm/`** — LLMClient (OpenAI-compatible async wrapper with retry + centralized per-call token tracking; Wave-6: `chat`/`chat_with_tools`/`chat_json` accept named `caller_tag` kwarg, recorded on `LLMCallRecord` and as span attribute `gen_ai.caller`)
...
| `SUBAGENT_MAX_TASK_DESCRIPTION_LENGTH` | `2000` | Wave-4 L2: SubAgentTool truncates oversized `task_description` before passing to SubAgent.run() |
| `SUBAGENT_ITERATION_EVENT_VERBOSITY` | `summary` | Wave-4 L5: `subagent_iteration` UI render mode — `summary` (every Nth iter) / `full` / `silent` |
| `SUBAGENT_ITERATION_EVENT_EVERY_N` | `2` | Wave-4 L5: when verbosity=summary, render every Nth iteration |
...
## System Prompt Composition (v12/v13/Wave-2)

`agents/prompt_utils.build_system_prompt(base, inject_context=True, inject_subagent_guidance=True, inject_location_guidance=True, inject_search_guidance=True, inject_hitl_guidance=True)` is the single composition entry point. Wave-2 moved the call site from **module-level constant** (frozen at import time) to **agent `__init__`** for ExecutorAgent / EmergentPlannerAgent / GoalDrivenPlannerAgent / PlannerAgent + private `_BASE_PROMPT` consts retained. SubAgent additionally passes `inject_hitl_guidance=False` (Wave-2 M1) since SubAgentTool structurally excludes `ask_user` from its whitelist. Reflector still uses a raw string (no builder).

Composition appends:

1. **Context injection** (v12, default ON) — current date / weekday / time. Now picks up "today" at agent-instantiation time rather than process-start time.
...
4. **SubAgent tool guidance** (v9, only when `SUBAGENT_ENABLED=true`).
5. **HITL `ask_user` guidance** (v13, only when HITL is active) — checks `_HITL_RUNTIME_OVERRIDE` first (set per-instance by `OrchestratorAgent.__init__`), falls back to `config.HITL_ENABLED`. Wave-2 fix: this override now actually wins because system prompts are composed at agent `__init__` (after Orchestrator has set the override), not at module import (where override is still `None`).

`build_convergence_hint(tool_call_counts)` is the single source of dynamic NOTE/CRITICAL nudges; called by all three ReAct loops (ReActEngine, GoalDrivenPlanner, EmergentPlanner legacy).
...
- `LLMCallRecord` carries `engine` (model id) **and** `caller_tag` (Wave-6 — agent identity, e.g. `"ExecutorAgent"` / `"GoalDrivenPlanner"` / `"SubAgent-2"`)
- `get_call_records()` — returns a copy
- `reset_usage()` — clears for a new task
- `Orchestrator._finalize_token_usage()` — aggregates into THREE views: `total`, `by_engine` (preserved), `by_caller` (Wave-6, new). `by_caller` total always equals `by_engine` total — same source. `caller_tag=""` records fall into `"unknown"` bucket so wiring gaps are visible
- UI renders both per-engine and per-caller tables (the latter only when ≥2 callers exist OR the only caller is non-`unknown`)
...
- **Per-caller LLM span attribute** (Wave-6): every `llm.*` span gets a `gen_ai.caller` attribute set from the caller_tag passed by the call site (e.g. `gen_ai.caller=SubAgent-2`). Combined with `subagent.parent_agent` on `subagent.execute.*` spans, traces show end-to-end attribution: which agent issued each LLM call and how it nests under SubAgent dispatch.
...
- **HITL isolation (v13)**: SubAgent's tool whitelist also structurally excludes `ask_user` (3 filter sites in `tools/subagent_tool.py`); Wave-2 M1 also excludes the `ask_user` system-prompt guidance via `inject_hitl_guidance=False` — SubAgents should report ambiguity via their structured summary, not by prompting the user directly.
- **Sandbox lifecycle (Wave-4 M3)**: `SubAgentTool.reset_task_state()` (called from `OrchestratorAgent.run()` per task) **deletes `subagent_<digits>` subdirectories** under `SANDBOX_DIR` from the previous task before counters reset. Files written outside `subagent_*` (i.e. directly in `SANDBOX_DIR`) survive across tasks. This prevents "ghost context" where a freshly-numbered SubAgent-1 in task B sees leftover files written by SubAgent-1 in task A.
- **Description bound (Wave-4 L2)**: SubAgentTool truncates `task_description` exceeding `SUBAGENT_MAX_TASK_DESCRIPTION_LENGTH` (default 2000) before passing to `SubAgent.run()`, with a clear `[Description truncated...]` marker — prevents over-eager parent LLMs from blowing up the SubAgent's own context window.
- **Failure-path log recovery (Wave-3 M2)**: `SubAgent._failure_tool_calls()` reads `self._react_engine._current_log` first (live mid-iteration data) and falls back to `self._accumulated_tool_calls` (last `on_iteration` snapshot). Timeout / token-budget / unexpected-exception summaries no longer lose the in-progress iteration's tool calls.
- **Honest unexpected-structure summary (Wave-3 M6)**: `_summarize_result` checks the LLM response dict for ANY recognized SubAgentSummary key before accepting it; pydantic's all-default-fields-pass behavior (which would silently accept `{"foo":"bar"}` as an empty-but-valid summary) is explicitly defeated. Repr goes into `issues`; `accomplished` stays empty rather than lying.
...
20. **Three ReAct loops, one shared helper module (Wave-1)**: ReActEngine remains canonical, but `GoalDrivenPlannerAgent._execute_todo_goal_guided` and `EmergentPlannerAgent._execute_todo` (legacy `use_react_engine=False` path) keep their own loops because each owns prompt-shape concerns the engine doesn't (goal injection, sliding window, flat history). To prevent behavioral drift, all three import `react/tool_call_helpers.py` and call `attribute_caller` / `classify_result` / `truncate_for_llm` at the same logical points. Adding a fix to one loop now means changing the helper, not patching three call sites. `ENABLE_REACT_ENGINE_V2` flag is still in config (backward compat) but no longer affects behavior. A v14-class refactor that folds GoalDrivenPlanner into ReActEngine via `pre_iteration_hook` / `message_window_strategy` extension points is out of scope for v9 SubAgent fixes.
21. **ToolRouter three-state accounting via shared helper (Wave-1)**: `react.tool_call_helpers.classify_result(result, exc)` returns `(is_error, is_rate_limited)` and is the single classification point used by all three ReAct loops. Three-state precedence: `rate_limited > error > success` → `record_rate_limited()` when an `Error:` payload contains `RATE_LIMITED_MARKER` ("SubAgent call limit reached"); `record_failure()` on any other `Error:` or raised exception; `record_success()` otherwise. Without this, tools that swallow exceptions and return Error strings (web_search / fetch_url / ask_user timeout) would be silently counted as success, and SubAgent rate-limit returns would pollute `failure_threshold` and trigger misleading "switch to alternative tool" hints (rate-limited tools have no alternative). The classifier must run AFTER the call so the result string is available — the prior `record_success()`-before-Error-check pattern (now extinct) caused that bug.
...
30. **Per-caller token attribution (Wave-6)**: `LLMCallRecord.caller_tag` identifies the agent that issued each LLM call. Set automatically by `BaseAgent.think_*()` (defaults to `self.name`, kwargs override) and explicitly by the three direct `llm_client.*` callers (`ReActEngine` → `self.agent_name`, `GoalDrivenPlannerAgent._execute_todo_goal_guided` → `"GoalDrivenPlanner"`, `SubAgent._summarize_result` → `self.name`). `Orchestrator._finalize_token_usage()` builds two parallel views: `by_engine` (preserved, unchanged) and `by_caller` (new). Records lacking a caller_tag (legacy traces) fall into the `"unknown"` bucket so any wiring gap is visible rather than silent. LLM spans in tracing carry `gen_ai.caller`. Evaluation in `evaluation/runner.py` prefers caller_tag aggregation for SubAgent token totals (single source of truth) and falls back to event-derived sums when caller_tag is empty (pre-Wave-6 traces). The caller_tag parameter is a named kwarg on `chat`/`chat_with_tools`/`chat_json` — never put it in `**kwargs` because it would leak to the OpenAI API.
31. **System prompts built per-instance, not at module import (Wave-2)**: Module-level `EXECUTOR_SYSTEM_PROMPT` / `EMERGENT_PLANNER_SYSTEM_PROMPT` / `V8_GOAL_DRIVEN_SYSTEM_PROMPT` / `SIMPLE_PLANNER_SYSTEM_PROMPT` / `PLANNER_SYSTEM_PROMPT` constants no longer exist. Each agent's `__init__` calls `build_system_prompt(_BASE_PROMPT, ...)` and stores the result on `self.system_prompt`. Two reasons: (a) `build_context_injection()` reads "today" — module-level evaluation freezes the date for the life of the process; (b) HITL guidance gating depends on `_HITL_RUNTIME_OVERRIDE` which `OrchestratorAgent.__init__` sets — module-level evaluation runs before the override is in scope and silently breaks the v13 double-gating contract. Tests asserting these consts existed were rewritten to call `build_system_prompt(_BASE)` directly. `Planner` `create_plan()` / `replan()` rebuild via `_build_simple_planner_prompt()` / `_build_planner_prompt()` instead of toggling between two module constants.
32. **ReActEngine `_current_log` rebound, never cleared (Wave-3)**: `_current_log` is exposed as a member attribute so `SubAgent._failure_tool_calls()` can read mid-iteration data (timeout / budget / exception paths). Each `execute()` invocation creates a FRESH local list and rebinds `self._current_log` to it — does NOT `clear()` the previous list. Reusing+clearing was a real concurrency bug under `DAG_SERIAL_EXECUTION=false`: `ExecutorAgent.create_for_node()` shares `self._react_engine` across parallel nodes, so a second `execute()` entering its `clear()` would wipe the list a still-running first `execute()` was appending to (and which the returned StepResult held a reference into). Pydantic does copy list fields on model construction, so already-returned StepResults are safe; but the in-flight task wasn't. SubAgent path is unambiguous because each SubAgent owns a private ReActEngine — at most one `execute()` in flight per engine.
33. **`set_caller` local capture is load-bearing under Wave-4 M4 (Wave-1 N5)**: `SubAgentTool.execute()` immediately copies `self._parent_name` to `local_parent` before any await. Wave-4 M4 introduced exactly such an await later in the function (`await asyncio.to_thread(os.makedirs, ...)`); during this await another task on the same shared `SubAgentTool` instance can call `set_caller(...)` and overwrite `self._parent_name`. The local snapshot ensures the SubAgent that's eventually constructed is attributed to the agent that actually invoked us, not whoever wrote `self._parent_name` last. What was originally a "defensive future-proof" snapshot in Wave-1 became actively load-bearing once Wave-4 added the makedirs await. Test: `test_local_parent_survives_await_with_concurrent_set_caller`.
34. **Latent circular import — `react.engine` ↔ `agents.prompt_utils` (Wave-3 fix)**: `react.engine` needs `build_convergence_hint`, but importing it triggers `agents/__init__.py` which eagerly imports `.subagent`, which imports `react.engine` again — partial-init `ImportError`. Top-level test ordering happened to dodge this; direct probes (`python -c "from react.engine import ReActEngine"`) hit it. Fix: import `build_convergence_hint` lazily inside `ReActEngine.execute()` where it's used, not at module top. After first call it's cached in `sys.modules`. **Don't move it back to top-level.**
