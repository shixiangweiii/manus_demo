...
Multi-agent mechanism following the Codex Subagent pattern. When `SUBAGENT_ENABLED=true`, the `subagent` tool is injected into the ExecutorAgent's tool list; the LLM can then decide to call it during ReAct loops.
