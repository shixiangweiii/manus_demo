...
- 工具集是**静态定义**的: `web_search`、`execute_python`、`file_ops`、`execute_shell`
...
- 4 个工具: `web_search`(mock)、`execute_python`(subprocess 沙箱)、`file_ops`(沙箱目录读写)、`execute_shell`(沙箱 Shell 命令执行)
...
- ✅ Shell 命令执行已通过 `ShellTool` 实现,支持安装依赖、运行项目、操作 git
- 没有浏览器能力,无法抓取网页、填写表单
...
- P0:新增 **shell 命令执行工具**(`asyncio.create_subprocess_exec`,支持超时和输出捕获) ✅ 已完成
