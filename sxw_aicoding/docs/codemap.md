...
v9 → SubAgent 多智能体协作（Claude Code Subagent 模式 + depth=1 结构隔离 + 纯摘要返回 + Token 预算熔断 + 反模式防御 + Tracing 集成 + Trace 详情页交互优化）
```
...
- **SubAgent 多智能体协作**：v9 新增 Claude Code Subagent 模式，depth=1 结构隔离 + 纯摘要返回 + Token 预算熔断 + 反模式防御
...
- **Web Viewer**：内置 FastAPI 可视化查看器，暗色主题树形 Span 展示 + 全屏模态框 + Copy 按钮 + LLM 属性优先渲染
...
│   ├── orchestrator.py       # OrchestratorAgent 中央协调者 (620行)
│   ├── planner.py            # PlannerAgent 混合规划器 (1118行)
│   ├── executor.py           # ExecutorAgent ReAct执行器 (350行)
│   ├── reflector.py          # ReflectorAgent 反思验证器 (254行)
│   ├── emergent_planner.py   # EmergentPlannerAgent 隐式规划器 (706行)
│   ├── goal_driven_planner.py # GoalDrivenPlannerAgent 目标驱动规划器 (v8, 957行)
│   └── subagent.py           # SubAgent 隔离式子智能体 (v9, 475行)
│
├── dag/                       # DAG 执行引擎
│   ├── __init__.py
...
├── llm/                       # LLM 客户端
│   ├── __init__.py
│   └── client.py             # LLMClient OpenAI兼容封装 (480行)
│
├── tools/                     # 工具系统
│   ├── __init__.py
│   ├── base.py               # BaseTool 工具基类 (174行)
...
│   └── subagent_tool.py      # SubAgentTool 子智能体元工具 (v9, 223行)
│
├── memory/                    # 记忆系统
│   ├── __init__.py
...
│   ├── metrics.py            # 指标模型 + 评分函数 (474行)
│   ├── benchmark.py          # 12 个基准任务 (310行)
│   ├── runner.py             # EvaluationProbe + EvaluationRunner (594行)
...
**文件**: `agents/orchestrator.py` (620行)
...
- v9 集成：当 SUBAGENT_ENABLED=true 时，SubAgentTool 注入工具列表
...
**文件**: `agents/emergent_planner.py` (706行)
...
**文件**: `agents/goal_driven_planner.py` (957行)
...
**文件**: `agents/subagent.py` (475行)

**目的**: 遵循 Claude Code Subagent 模式的隔离式子智能体。depth=1、独立上下文、纯摘要返回。

**与 v5/v8 的关键区别**:
- depth=1: 工具白名单结构性排除 `subagent`，不能嵌套派生
- 独立上下文: 自己的 ReActEngine、messages 列表、system prompt
- 纯摘要返回: 父智能体只接收 SubAgentSummary JSON，不获取完整对话历史
- Token 预算熔断: on_iteration 回调检测累计 token 超限，触发 SubAgentTokenExhausted 异常
- 沙箱隔离: 可选 per-SubAgent 沙箱子目录防止双写冲突
...
```mermaid
graph TD
    A[SubAgentTool.execute] --> B[验证白名单 + 创建沙箱]
    B --> C[创建SubAgent]
    C --> D[ReActEngine.execute + on_iteration]
    D --> E{成功?}
    E -->|是| F[_summarize_result → SubAgentSummary]
    E -->|否| G[fallback_summary]
    E -->|Token超限| H[SubAgentTokenExhausted → budget_summary]
    E -->|超时| I[TimeoutError → timeout_summary]
    F --> J[SubAgentResult COMPLETED]
    G --> K[SubAgentResult FAILED]
    H --> K
    I --> L[SubAgentResult TIMED_OUT]
    J --> M[返回 summary_text JSON]
    K --> M
    L --> M
```
...
#### SubAgentTool (v9)
**文件**: `tools/subagent_tool.py` (223行)

**目的**: 为复杂子任务派生子智能体的元工具。当 LLM 在 ReAct 循环中调用 `subagent(task_description, tool_whitelist?)` 时，创建隔离式 SubAgent 执行子任务，返回结构化摘要 JSON。
...
- v8 新增事件处理器：goal_anchor、goal_reflection、goal_reanchor
- v9 新增事件处理器：subagent_start、subagent_complete、subagent_failed、subagent_timed_out
...
### 6+2. SubAgent 多智能体协作 (v9)

**描述**: SubAgent 采用 Claude Code Subagent 模式的多智能体协作，通过 depth=1 结构隔离 + 纯摘要返回 + Token 预算熔断实现安全的多智能体委派。

**实现位置**: `agents/subagent.py`: `SubAgent` 类, `tools/subagent_tool.py`: `SubAgentTool` 类

**优势**:
- depth=1 结构隔离：白名单过滤排除 subagent 工具，LLM 无法嵌套派生
- 独立上下文：自己 ReActEngine + messages + system prompt，防止上下文泄漏
- 纯摘要返回：父智能体只接收 SubAgentSummary JSON，不获取完整对话历史
- Token 预算熔断：on_iteration 回调检测累计 token 超限，触发 SubAgentTokenExhausted
- 沙箱隔离：可选 per-SubAgent 工作目录防止双写冲突
- 调用次数限制：SUBAGENT_MAX_CALLS_PER_TASK 限制单任务 SubAgent 派生次数

**核心循环**:
```python
# SubAgentTool.execute() (called by LLM during ReAct)
task_description = kwargs["task_description"]
tool_whitelist = kwargs.get("tool_whitelist", [])
validated_whitelist = [n for n in tool_whitelist if n != "subagent" and n in available_tools]

subagent = SubAgent(name=..., task_description=task_description, tools=restricted_tools)
...
| 文件路径 | 行数 | 核心职责 | 关键类/方法 |
|---------|------|---------|-----------|
| `agents/orchestrator.py` | 620 | 中央协调者，三路由管理（含v8+v9） | `OrchestratorAgent.run()` |
| `agents/planner.py` | 1118 | 混合分类器 + 计划生成 | `PlannerAgent.classify_task()` |
| `agents/executor.py` | 350 | ReAct 执行器 | `ExecutorAgent._react_loop()` |
| `agents/reflector.py` | 254 | 质量验证与反馈 | `ReflectorAgent.reflect()` |
| `agents/emergent_planner.py` | 706 | 隐式规划器 | `EmergentPlannerAgent.execute()` |
| `agents/goal_driven_planner.py` | 957 | 目标驱动规划器 (v8) | `GoalDrivenPlannerAgent.execute()` |
| `agents/subagent.py` | 475 | 隔离式子智能体 (v9) | `SubAgent.run()` |
| `agents/base.py` | 182 | 智能体基类 | `BaseAgent.think()` |
| `dag/executor.py` | 702 | Super-step 并行执行 | `DAGExecutor.execute()` |
| `dag/graph.py` | 626 | DAG 数据结构 | `TaskDAG.get_ready_nodes()` |
| `dag/state_machine.py` | 113 | 节点状态机 | `NodeStateMachine.transition()` |
| `llm/client.py` | 480 | LLM 客户端 | `LLMClient.chat()` |
| `react/engine.py` | 262 | 统一 ReAct 引擎 | `ReActEngine.execute()` |
...
| `tools/subagent_tool.py` | 223 | 子智能体元工具 (v9) | `SubAgentTool.execute()` |
...
| `evaluation/metrics.py` | 474 | 评测指标模型 + 评分函数 | `compute_overall_score()`, `aggregate_results()` |
| `evaluation/benchmark.py` | 310 | 12 个基准任务定义 | `get_benchmark_tasks()` |
| `evaluation/runner.py` | 594 | 事件探针 + 评测执行器 | `EvaluationProbe`, `EvaluationRunner` |
| `evaluation/report.py` | 308 | Rich 报告 + JSON 导出 | `render_comparison_table()`, `export_json()` |
| `evaluation/eval_cli.py` | 185 | 评测 CLI 入口 | `main()` |
| `tracing/__init__.py` | 66 | 模块入口 + No-op Stubs | - |
| `tracing/__main__.py` | 107 | Web Viewer CLI 入口 | - |
| `tracing/config.py` | 78 | Tracing 配置集中管理 | - |
| `tracing/spans.py` | 265 | Span/Attribute/Event 语义常量 | - |
| `tracing/provider.py` | 196 | TracerProvider 工厂 | - |
| `tracing/exporters.py` | 304 | FileSpanExporter + RichConsoleExporter | - |
| `tracing/decorators.py` | 145 | @traced / @traced_llm_call / @traced_tool_call | - |
| `tracing/bridge.py` | 885 | TracingBridge 事件→Span 桥接器 | - |
| `tracing/server.py` | - | Web Viewer FastAPI 应用 | - |
| `tracing/server.py` | 275 | Web Viewer FastAPI 应用 | - |
| `schema.py` | 749 | 数据模型定义（含v8 GoalDocument + v9 SubAgent模型） | `Plan`, `TaskDAG`, `TaskNode`, `GoalDocument`, `SubAgentResult` |
| `config.py` | 120 | 全局配置（含v8目标驱动+v9子智能体配置） | `LLM_MODEL`, `MAX_PARALLEL_NODES`, `ENABLE_GOAL_DRIVEN_PLANNER`, `SUBAGENT_ENABLED` |
| `main.py` | 572 | CLI 入口 | `main()` |
...
| v9 | `agents/subagent.py`, `tools/subagent_tool.py`, `schema.py`, `config.py`, `agents/orchestrator.py`, `tracing/spans.py`, `tracing/bridge.py`, `tracing/templates/trace_detail.html` | SubAgent 多智能体协作（depth=1 + 纯摘要返回 + Token 预算熔断 + 反模式防御）+ Trace 详情页交互优化 |
...
| `tests/test_subagent.py` | v9 SubAgent 测试（1689行，91 用例：数据模型、SubAgent核心逻辑、SubAgentTool、Orchestrator注入、EvaluationProbe、TracingBridge、反模式防御） |
...
7. **SubAgent 多智能体协作**：v9 新增 Claude Code Subagent 模式，depth=1 结构隔离 + 纯摘要返回 + Token 预算熔断 + 反模式防御
8. **LLM 重试机制**：v6 新增的指数退避重试，提升系统稳定性
9. **Shell 命令执行**：v6 新增 `ShellTool`，支持在沙箱中执行 bash 命令，增强真实环境交互能力
10. **全链路 Tracing**：v7 新增基于 OpenTelemetry 的结构化 Span 树，覆盖 LLM 调用、工具执行、反思、SubAgent 等全生命周期，支持 File/Rich/OTLP/Phoenix 多后端导出，并提供内置 Web Viewer 可视化查看器（含全屏模态框 + Copy 按钮 + LLM 属性优先渲染）

该系统展示了如何将多种先进技术（ReAct、DAG、状态机、混合路由、目标驱动规划、SubAgent 多智能体协作等）整合为一个实用的多智能体系统，为复杂任务的自动化执行提供了完整的解决方案。
