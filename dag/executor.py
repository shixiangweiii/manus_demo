...
class DAGExecutor:
    ...
    def _should_adapt(self, step: int, dag: TaskDAG) -> bool:
        """
        Decide whether to invoke adaptive planning at this super-step.
        决定在当前超步是否触发自适应规划。

        Conditions:
          1. Interval check: only adapt every N super-steps
          2. Minimum completed: at least M actions must have completed
          3. Pending nodes exist: there must be something to adapt
    ...
    async def _adapt_plan(self, step: int, dag: TaskDAG) -> None:
        ...
        changes = self._planner.apply_adaptations(dag, adaptation.adaptations)
